      ...
      <div className="bg-gray-800 h-screen hide-scrollbar overflow-y-auto w-64 fixed flex flex-col gap-7 items-center">
        ...
        <div className="flex flex-col mt-20 gap-8 items-center">
          <div className="flex flex-col gap-1 items-center">
            ...
            <Text size="xs" c="dimmed" className="text-white">{user.role}</Text>
          </div>
            ...
            {links.map((link) => {
              ...
              return (
                <div key={link.name} className="w-full">
                  <Link
                    to={link.url}
                    className={`group flex items-center gap-3 px-4 py-2 rounded-md w-48 transition-all 
                      ${isActive ? "bg-green-400 text-black" : "text-white hover:bg-green-300 hover:text-black"}
                    `}
                  >
                    <span className={`${isActive ? "text-black" : "text-white"}`}>
                      {link.icon}
                    </span>
                    <span className={`font-medium ${isActive ? "text-black" : "text-white group-hover:text-black"}`}>
                      {link.name}
                    </span>
                  </Link>
                </div>
